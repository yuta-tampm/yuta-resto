# Gate 1 — Repository Format Policy
...
Control Tower cho phép mở change và Proposal/Analysis only theo direction A_FORMAT_SCOPE_WITH_EXPLICIT_ALTERNATE_VALIDATION. Đây không phải Gate 1 approval hoặc permission cho exclusions/implementation. Gate-1 packet recommendation: READY_FOR_GATE_1 — ready for decision review, NOT ready for Specs.

Sources, ten-domain impact matrix, seven conceptual classes, generated options, historical/archive constraints và exact decision questions được giữ nguyên trong artifact snapshots bên dưới. Các câu hỏi requirement-level phải có câu trả lời tường minh và approved artifact revision trước khi Specs.

## Integrity

HEAD trước tạo change: `415990386327aaccab3c32b1fef0569a0fde7f3a`. Existing dirty Pointage, parent integration và Formalités README work được giữ ngoài scope. Intended new files: CLI-created .openspec.yaml, proposal.md, analysis.md, packet này. Không tạo Specs/Design/Tasks.

Hashes dùng Node crypto SHA-256 trên fs.readFileSync exact bytes, lowercase hex:
...
Parent Tasks protected hash: `36e66b08a712d99a12f3f3243a803bced9bf707a7bb6ea9b47b83ec7e896b504`. Parent vẫn 19/23, 5.4 COMPLETE, 6.1 FAIL/UNCHECKED, 6.2–6.4 BLOCKED, receipt VERIFIED. Production NOT_AUTHORIZED.

## Validation evidence

- `openspec new change "repository-format-policy-and-baseline-remediation"`: exit 0. Progress banner nói spec-driven, nhưng final result, metadata và status xác nhận yuta-spec-driven; không đổi schema.
- `openspec status --change "repository-format-policy-and-baseline-remediation" --json`: exit 0; đúng local root/schema.
- Proposal/Analysis instructions: exit 0; followed returned paths/dependencies.
- `openspec validate repository-format-policy-and-baseline-remediation --strict --no-interactive`: exit 1, "Change must have at least one delta. No deltas found." Specs chưa được phép tạo tại Gate 1. Không set skip_specs vì đây là observable validator behavior change; không claim strict PASS.
- `pnpm docs:check`: exit 0, 36 current documents.
- `pnpm architecture:check`: exit 0.
- Scoped Prettier Proposal/Analysis: exit 0.
- Scoped git diff --check: exit 0; new untracked files cần bổ sung explicit content checks vì git diff không tự bao gồm chúng.
- Không chạy lint, runtime tests/build, recursive typecheck, global format remediation hoặc parent closure; scope chỉ planning docs.

## Gate 1 decisions

DECIDED_FROM_AUTHORITY: bounded direction A; fail-closed classification; no silent baseline exclusions or old-approval reuse; no manual generated formatting; normative behavior/lifecycle preservation; WAIT_FOR_OWNER; mandatory Sensitive Design.

NEEDS_CONTROL_TOWER_DECISION:

1. Archive exact-byte preservation and correction via append/new revision.
2. Historical-class admission, mutable-format exclusion and required preservation validator.
3. Generated policy preference and required full-pipeline/deviation evidence.
4. Mandatory complete coverage and alternate validation; concrete manifest/representation and orchestration placement deferred to Design.
5. Active ownership admission/exit coordination, no hygiene takeover.
6. Exact 67-file migration preimages, stop-on-drift and owning approval for mutable subsets.

General historical rebind migration remains NOT_CURRENTLY_ESTABLISHED; archive byte policy remains UNSPECIFIED until answered. Approval word alone does not resolve these questions.
...
AWAITING_HUMAN_REVIEW. Không Specs, Design, Tasks/TIC, Apply, Sync, Archive, Knowledge Consolidation hoặc production. Không tiến parent.
