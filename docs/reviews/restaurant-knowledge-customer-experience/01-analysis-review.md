...
Created: 2026-09-01T00:07:30.4935205+02:00
...
Approved: 2026-09-01T00:12:34.4302099+02:00
...
`/etablissement/informations-generales`: Restaurant Knowledge `Expérience
client`, gồm `Expérience souhaitée`, `Accueil & service` và `Attention
particulière au client`.
...
Organization chỉ là tenancy/access envelope. Slice không sở hữu, đọc, ghi, suy
luận, liên kết hoặc đồng bộ dữ liệu Reservations, Reputation, Today, Personnel,
POS/orders, CRM/customer profile, Marketing/social, external provider hay
Restaurant Knowledge section khác. Proposal không quyết định schema, table,
repository, API, shared contract hoặc Product validation.
...
The shared worktree was already dirty with the completed and archived
Cuisine/savoir-faire implementation and its normative spec before this change
was created. Gate 1 preserves those paths byte-for-byte. Paths attributable to
this change are limited to its new OpenSpec directory and this review directory;
no runtime, schema, migration, Product Knowledge, normative spec, Design, Tasks
or implementation file was changed.

## Authorities consulted

- Root, Backoffice and cloud-database scoped instructions.
- Authority Model, OpenSpec activation/normativity policies, Lifecycle Status
  Model and the Restaurant Knowledge Module Registry row.
- ADR-007, Establishment Product Knowledge and the
  `Informations générales` page Product Knowledge.
- Tenancy, Authentication, Identity/Membership and Database Boundaries.
- Normative specs for Restaurant Knowledge authorization, Concept/Histoire and
...
- Current page composition, authorization guards, loaders/actions/forms,
  Restaurant Knowledge schema/repository and focused authorization/tenant tests.

## Current compatibility finding

The existing Restaurant Knowledge infrastructure is compatible with the
approved slice without changing any accepted boundary:

- READ and MANAGE already exist as separate operations and carry the approved
  OWNER/MANAGER grants with STAFF denial.
- Trusted active organization + establishment membership enforcement remains
  unchanged and server-derived.
- Current Restaurant Knowledge reads/writes are scoped by organization and
  establishment.
- The page already composes independent Restaurant Knowledge slices without
  merging them into Establishment Profile.
- Scoped source inspection found no dependency from current Restaurant
  Knowledge code to Reservations, Reputation, Today, Personnel, POS/Site Agent,
  Marketing, providers, embeddings or vector processing.

The existing Concept/Histoire and Cuisine/savoir-faire implementations are
evidence of compatible boundaries, not authority to copy their technical
representation mechanically.

## Conflicts and NEEDS REVIEW

- **CONFLICT — non-blocking documentation drift:** the broad
  `docs/PRODUCT_KNOWLEDGE.md` routing text still calls Restaurant Knowledge
  not-started with unresolved ownership/permissions. The Establishment Product
  Knowledge OpenSpec section also retains stale pre-sync wording for
  Cuisine/savoir-faire. The more specific approved page Product Knowledge,
  Module Registry, current working-tree implementation and three normative main
  specs establish the current accepted state. This drift concerns documentation
  status/routing, not the requirements of `Expérience client`, and must be
  corrected only in a later approved documentation scope.
- **NEEDS REVIEW — deferred to Design:** exact persistence shape, table/repository
  representation, page-local component boundary and migration/rollback mechanics
  are not Product decisions. Design may select them only inside the accepted
  Restaurant Knowledge cloud/domain boundary.
- No remaining requirement-level ambiguity, authorization gap, cross-module
  dependency or durable-boundary conflict blocks Specs.

## Product/authority questions requiring answers

None before Specs. If later Design or Apply requires a shared contract, new
permission, changed tenancy semantics, changed canonical ownership,
Reservations/Reputation/Today/Personnel/POS dependency, external provider or
...
Gate 1 promotes none of these values.
...
`restaurant-knowledge/customer-experience`. Do not create Design, Tasks or
implementation before their normal approval gates.
