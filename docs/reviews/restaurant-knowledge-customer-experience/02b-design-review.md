...
Created: 2026-09-01T00:26:25.5041819+02:00
...
Approved: 2026-09-01T14:58:47.7800420+02:00
...
- Add one dedicated Restaurant Knowledge cloud table with composite
  organization/establishment scope and exactly three nullable descriptive
  values.
- Keep missing persistence and all-null persistence as the same valid all-empty
  projection; save all three values atomically with one scoped upsert.
- Reuse the existing trusted tenant context and separate Restaurant Knowledge
  READ/MANAGE guards without Profile permission inheritance.
- Add an independent route-local loader, action, draft model and form with one
  explicit submit and no autosave trigger.
- Keep Reservations, Reputation, Today, Personnel, POS/orders, Marketing, CRM,
  AI/providers and all external runtimes outside reads, writes, links and
  consumers.
- Attribute Apply changes from exact pre-Apply bytes because the shared
  worktree was already dirty before this change.

## Security, data, and runtime implications

- Restaurant Knowledge remains canonical owner. The data is semantically
  establishment-scoped; Organization remains only the tenancy/access envelope.
- Composite primary/foreign scope and every repository predicate use both
  `organizationId` and `establishmentId` from trusted server context.
- Existing active user, organization, establishment and membership resolution
  remains unchanged and fail-closed. Browser scope/role/permission values are
  not accepted.
- `restaurant-knowledge.read` gates repository loading and view;
  `restaurant-knowledge.manage` separately gates edit/save. No new permission,
  role, principal or admin/support bypass is introduced.
- Establishment Profile data and permission contracts remain independent.
- Runtime remains cloud Backoffice plus `@yuta/db-cloud`; there is no public,
  POS/Site Agent, Display, provider or cross-runtime path.

## Migration and rollback

The migration is additive: one dedicated cloud table with three nullable text
columns and existing composite establishment-scope constraints. No existing
table or data is transformed, and no deployed migration is edited.

The migration must be executed on a disposable PostgreSQL database and reviewed
before Gate 3. Deployment order is migration before application dependency.
An earlier application version ignores the additive table.

Application rollback removes use of the table while retaining any entered data.
This change does not create a destructive down migration or automatic table
drop. Any future removal requires a separately reviewed migration and data
decision.

## Unresolved choices and stop-condition review

No unresolved choice affects Specs, approach or task breakdown. The generated
migration filename is tooling-owned and does not change the Design.

The Design does not require a shared contract, new permission, changed tenancy
boundary, changed canonical ownership, prohibited module dependency, external
provider or cross-runtime behavior. Therefore no stop condition was triggered.

## Dirty-worktree attribution checkpoint

All dirty files outside this change's OpenSpec and review directories were
recomputed as the same deterministic sorted `status|path|SHA-256` manifest
recorded at Gate 2:
...
Therefore the Design stage did not alter or attribute any pre-existing dirty
file. Apply must capture a new exact baseline only after Gate 2b approval.
...
Only `design.md`, the Gate 2 approval metadata, and this mandatory Design Gate
packet were changed after Gate 2 approval. No Tasks, implementation, migration,
normative sync or archive action was performed.

## Recommendation and required human action

Recommendation: approve Gate 2b only if the dedicated Restaurant Knowledge
persistence boundary, page-local server/UI composition, authorization/tenant
enforcement, migration/rollback plan and explicit module exclusions preserve
the reviewed Product contract.

Approval authorizes creation of Tasks followed by Apply/Verify under this exact
Design. It does not authorize normative sync or archive.
