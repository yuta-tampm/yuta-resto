...
Gate: Design Review — explicitly requested after Gate 2
Review status: `APPROVED`
Approval source: `explicit current-user instruction`
Approval recorded by: `Codex workflow`
Approved: `2026-09-06T18:33:35.1730885+02:00`
Created: `2026-09-06T18:21:52.6901067+02:00`
Schema: `yuta-spec-driven`
Analysis conclusion: `READY_FOR_SPECS`
Sensitive change: `NO`

Classification: `CROSS_MODULE`
Design applicability: `APPLICABLE`
Sensitive Design Gate assessment: `SENSITIVE_DESIGN_GATE_NOT_REQUIRED`
Design status: `done`
Tasks status: `AUTHORIZED`
Apply status: `NOT_AUTHORIZED`

This packet is the current-user-requested exact Design review. It is named `02b-design-review.md` for the requested repository review sequence, but the assessment below concludes that the workflow's sensitive-change criteria are not met.

## Approved Earlier Gates and Integrity

- Gate 1 packet: `docs/reviews/async-interaction-feedback-foundation/01-analysis-review.md`
- Gate 1 packet SHA256: `03cbb892029eb01c8bac8b48e7c301458e21acde4940c4c5f71f2a08eea6c15c`
- Gate 2 packet: `docs/reviews/async-interaction-feedback-foundation/02-specs-review.md`
- Gate 2 packet SHA256: `221fa3040d8c5756c5f00dd9a0d42ec18c314bb00c8f0b286c4f1c3eab628b92`
...
- Delta spec SHA256: `46840b69a936b96e9e6c49bee6330ea1a6206774f7aea7483dbabda4f159cb8e`
- Gate 1 and Gate 2 artifacts were recomputed and matched their reviewed hashes before Design.

## Architecture and Ownership Summary

- `packages/ui` owns only the domain-neutral native `Button loading` contract: disabled/busy semantics, preserved children/native attributes and direct contract tests.
- `apps/backoffice` owns pending copy/indicator composition and the bounded General Information, Google location and logout pilot presentation.
- Route/domain owners retain validation, authoritative result, redirect, conflict, uncertain operation, retry identity, idempotency and focus recovery.
- No global async provider, universal action component, new state library, toast convention or broad adoption is introduced.

## Shared Primitive Decision

- Add derived `aria-busy="true"` when `Button loading=true`; preserve caller-supplied `aria-busy` when loading is false.
- Preserve native `disabled={disabled || loading}`, `data-loading`, children, variants, refs and native attributes.
- Add no automatic spinner and no automatic label replacement.
- Normative pending guarantee applies only to native button rendering.
- `asChild + loading` remains unsupported for the normative guarantee; non-native consumers own equivalent activation and accessibility semantics.
- Current repository scan found no `asChild + loading` consumer.

## Pilot Allowlist

1. General Information standard form — baseline/non-regression; no architecture refactor.
2. Google Business location selection — route-local client submit button adds pending copy and duplicate prevention while preserving redirect-based Server Action outcomes.
3. Logout — shell-local submit button adds pending accessible meaning and duplicate prevention while preserving session revocation and redirect.
4. Formalités CDI draft workspace — compatibility evidence only; no production state-machine change.

## Shared Contract Test Ownership

- Test location: `packages/ui/test/button.test.tsx`.
- Owner: `@yuta/ui`.
- Runner: Vitest already used by repository workspaces.
- Exact full command after package-local wiring: `pnpm --filter @yuta/ui test`.
- Exact focused command after package-local wiring: `pnpm --filter @yuta/ui exec vitest run test/button.test.tsx`.
- New wiring required: package-local `test` script plus direct Vitest/`react-dom` test dev dependencies and lockfile update.
- Classification: local package tooling wiring, not a durable repository-wide testing convention.
- Control Tower review required for this decision: `NO`.

## Security, Data and Runtime Implications

- Authorization/security: unchanged.
- Trusted organization/establishment, membership, role and permission scope: unchanged.
- API/Server Action contracts: unchanged.
- Provider, idempotency, operation keys, replay, conflict/retry identity, transactions and persistence: unchanged.
- Runtime topology and redirect/revalidation architecture: unchanged.
- Stuck-pending runtime issue: `UNVERIFIED` and excluded.

## Compatibility, Migration and Rollback

Existing native `Button loading` consumers gain only programmatic busy semantics; visible children and disabled behavior remain unchanged. No current delegated-loading consumer requires migration.

No data/API/route migration exists. A code rollback would revert the shared busy attribute, package-local test wiring and the three bounded Backoffice presentation changes; Server Actions and persisted state are unaffected.

## Risks and Mitigations

- Duplicate assistive announcements → avoid global live regions and verify pilot routes in Browser QA.
- Server-rendered tests cannot prove navigation/click timing → combine direct contract tests, focused route tests and later Browser QA.
- Google provider mutation may be unsafe/unavailable → require safe development evidence or report `BLOCKED_BY_ENVIRONMENT`.
- Local test dependency drift → align with existing monorepo versions and lockfile.
- `asChild + loading` remains expressible → document unsupported boundary and keep zero current consumers.
...
`Foundation / Data` and `Service / Domain` are not applicable.

## Control Tower Stop Conditions

Stop with `CONTROL_TOWER_REVIEW_REQUIRED` if Tasks or implementation would require runtime completion/return-shape/redirect sequencing changes; authorization or tenant changes; provider/data/API/idempotency/conflict/transaction/persistence changes; a global async provider; or a durable repository-wide testing convention.

## Sensitive Design Gate Assessment

Result: `SENSITIVE_DESIGN_GATE_NOT_REQUIRED`.

Reason: Design is limited to backward-compatible frontend presentation semantics, smallest client boundaries and package-local tests. It changes no security, runtime/data ownership, migration, payment/fiscal, legal/privacy, provider contract, POS integrity, irreversible operation or cross-module durable boundary. Formalités is non-regression evidence only.

## Remaining Decisions

None. The Design resolves test ownership, native/delegated boundary, pilot targets, client boundaries, accessibility behavior, compatibility and stop conditions sufficiently for a later Tasks review.
...
Assessment: `PASS` — one change validated, zero issues.

Additional Design-artifact checks:

| Command                                                                                                                               | Result                                                                 |
| ------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `pnpm docs:check`                                                                                                                     | `PASS` — documentation consistency passed for 36 current documents.    |
| `pnpm exec prettier --write 'openspec/changes/async-interaction-feedback-foundation/design.md'`                                       | `PASS` — formatting applied only to the authorized Design artifact.    |
| `pnpm exec prettier --write 'docs/reviews/async-interaction-feedback-foundation/02b-design-review.md'` followed by targeted `--check` | `PASS` — Design and its review packet match repository Prettier style. |
...
Working-tree scope at review: only `openspec/changes/async-interaction-feedback-foundation/` and `docs/reviews/async-interaction-feedback-foundation/` are untracked. No unrelated path is reported by `git status --short`.

Implementation builds, tests, typechecks and Browser QA were not run because this gate is Design-only and no application or shared UI code was changed.

## Exact Design

Path: `openspec/changes/async-interaction-feedback-foundation/design.md`
...
| Artifact                                            | SHA256                                                             |
| --------------------------------------------------- | ------------------------------------------------------------------ |
| `design.md`                                         | `0aa7c42257e6e9e11827c1e41a1d8dd61e96cc5391561ba75e51cbe546224138` |
...
Approval means only: accept the exact Design above and authorize creation of Tasks/Implementation Plan at the next separately governed step. It does not authorize Tasks content in advance, Apply, production changes, sync or archive.

## Approval Record

The current user explicitly approved the exact Design SHA-256 above and authorized Tasks creation. This approval does not authorize Apply, production changes, sync or archive.
