...
Created: 2026-08-31T01:07:35.7050063+02:00
...
Approved: 2026-08-31T12:24:35.5039652+02:00

# Gate 2 — Specs Review

## Approved Gate 1 reference

Gate 1 was approved by the explicit current-user instruction `APPROVE Gate 1`.
Before creating Specs, the recorded Proposal and Analysis hashes were
recomputed and matched the approved packet.
...
```powershell
Get-FileHash -Algorithm SHA256 -LiteralPath 'openspec/changes/restaurant-knowledge-cuisine-know-how/specs/restaurant-knowledge/cuisine-know-how/spec.md'
```
...
The delta contains 9 ADDED requirements and 19 scenarios:

1. Restaurant Knowledge canonical ownership and establishment semantic scope,
   with Organization only as tenancy/access envelope and no Profile/Menu
   ownership.
2. View protected by Restaurant Knowledge READ without Establishment Profile
   permission inheritance.
3. Edit and explicit save protected by Restaurant Knowledge MANAGE, separate
   from READ.
4. Three independently optional values, including valid all-empty state and
   each single-value state.
5. Independent manual input/edit behavior.
6. One explicit save for the complete `Cuisine & savoir-faire` slice.
7. No persistence before explicit save.
8. No relationship, copy, link, sync, or mutation of `Carte & menus`/POS
   operational data.
9. No automatic enrichment or out-of-scope AI/review/Marketing/provider/vector
   integration.

The accepted normative `authorization/restaurant-knowledge` spec supplies the
grant matrix and tenant checks consumed here: OWNER and MANAGER receive READ
and MANAGE; STAFF receives neither by default; active user, organization,
establishment, matching active membership, and server-derived tenant context
remain required.
...
None. The Specs preserve the approved page-local Product decision, canonical
ownership, establishment semantic scope, Organization tenancy/access envelope,
READ/MANAGE behavior, optional independent values, valid empty state, one
explicit save, no autosave, `Carte & menus` separation, and all exclusions.

The Specs do not select a data type, schema, table/repository, API, validation
rule, taxonomy, shared contract, provider, or cross-runtime behavior.

## Remaining ambiguity

No requirement-level ambiguity remains. Technical persistence/data shape is
still deferred to Design. Any Design need for a shared contract, additional
permission, changed tenancy boundary, changed canonical ownership, `Carte &
menus` dependency, or cross-runtime behavior must return to review.

## Recommendation

Approve Gate 2 for the exact delta Spec above. Because the change is sensitive
