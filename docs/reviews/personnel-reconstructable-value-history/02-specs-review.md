...
Created: `2026-09-03T13:08:57.9008103+02:00`
...
Approved: `2026-09-03T13:54:09.5206538+02:00`

## Approved regenerated Gate 1 reference

The regenerated Gate 1 is approved in:

- `docs/reviews/personnel-reconstructable-value-history/01-analysis-review.md`
- Current approved Gate 1 packet SHA-256: `160e328f6c2683c5378e1cdcff01d8fb2a858065cb60b771e698dd8b964517a1`
- Approval source recorded there: explicit current-user instruction
- Approved: `2026-09-03T13:08:00.4192298+02:00`
- The earlier Gate 1 approval that permitted future effective dates is superseded for that rule.

The regenerated Gate 1 artifact hashes were recomputed before revising Specs and matched exactly:

| Gate 1 artifact                                                        | SHA-256                                                            |
| ---------------------------------------------------------------------- | ------------------------------------------------------------------ |
...
This packet supersedes the prior Gate 2 packet that was marked `CHANGES_REQUESTED`.

The targeted delta-spec revision:

- limits Role, Contract terms and Work time `CHANGE` effective dates to the past or current business date;
- rejects future effective dates for the entire mutation with no F07 history;
- explicitly excludes scheduled Personnel changes, pending future values and future activation;
- evaluates classification, effective-date requirements and correction-reason requirements independently per changed semantic group;
- rejects the whole multi-group mutation with no F07 history for any group if one changed group is invalid;
- preserves atomicity, revision-conflict and idempotency semantics.

No other approved Product boundary was changed. No Design, Tasks, schema, migration, API or implementation artifact was created.

## Delta spec integrity hash

The exact revised delta spec bytes were hashed with:

```powershell
Get-FileHash -Algorithm SHA256 -LiteralPath 'openspec/changes/personnel-reconstructable-value-history/specs/personnel/reconstructable-value-history/spec.md'
```
...
The revised delta defines 16 behavioral requirements with 49 testable scenarios:

1. The six approved semantic groups.
2. Independent per-group validation for multi-group mutations.
3. Distinct `CORRECTION` and `CHANGE` meanings.
4. OWNER-selected Identity classification and correction-only Entry.
5. Required effective dates limited to the past or current business date, with entry/departure boundaries and explicit future-date rejection.
6. The approved correction-reason matrix.
7. Authoritative previous/new values for complete semantic groups.
8. Atomic mutation, history and idempotency outcomes.
9. Eager exactly-once cutover baseline behavior.
10. The current-at-cutover limitation and prohibition on invented pre-cutover history.
11. OWNER-only reconstructable details in the existing `Historique`.
12. Preservation of the newest-50 window with no search/filter/pagination/export.
13. Compatibility with current departure recording, correction and cancellation.
14. Trusted tenant scope and current Personnel authorization.
15. The approved retention baseline and separately approved legal-hold override.
16. Explicit exclusions for Documents, Register, Formalités, AI, consultation and authentication history.

Scheduled Personnel changes, pending future values and future activation are explicitly outside F07.
...
```json
{
  "items": [
    {
      "id": "personnel-reconstructable-value-history",
      "type": "change",
      "valid": true,
      "issues": [],
      "durationMs": 13
...
Validation result: `PASS — 1/1 change valid, zero issues`.

## Changed assumptions since Analysis

No assumption changed after the regenerated Analysis. The targeted revision implements its approved no-future-effective-date rule and per-changed-group clarification.

No Product, ownership, authorization, tenancy, runtime, cutover, retention, departure, history-projection or module-boundary assumption changed.

## Remaining ambiguity and sensitive-design boundary

No requirement-level ambiguity remains.

The following remain intentionally unresolved for the mandatory Sensitive Design Gate after Gate 2 approval:

- storage model for immutable previous/new group values;
- atomic write and idempotency implementation details;
- eager exactly-once cutover execution and restart safety;
- retention eligibility, deletion/anonymization integration and legal-hold representation;
- migration, rollback and backup interaction;
- UI component and form-state implementation details.

The future scheduled/pending state model is outside F07 and MUST not be introduced by Design.

If Design requires a new owner, permission, shared history/retention contract, cross-module write, tenant/security change, cross-runtime behavior or Product change to retention/privacy, the workflow must stop and return to Control Tower.

## Recommendation

The revised delta spec matches the approved regenerated Proposal and Analysis and passes strict OpenSpec validation.

Recommendation: approve Gate 2 to permit technical design only. Because this is a Personnel/privacy-sensitive and data-affecting change, the next state must create `design.md` and a separate `02b-design-review.md`, then stop again. Gate 2 approval does not authorize schema, migration, API, tasks, implementation, production, sync or archive.
