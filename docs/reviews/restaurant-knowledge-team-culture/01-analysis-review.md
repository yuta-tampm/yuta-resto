...
Created: 2026-09-01T23:08:45.5817950+02:00
...
Approved: 2026-09-01T23:33:00.9717750+02:00

# Gate 1 — Analysis Review

## Request and bounded-change summary

Change page-local này bổ sung đúng một capability mới trên
`/etablissement/informations-generales`: Restaurant Knowledge `Équipe &
culture`, gồm `Valeurs & état d’esprit`, `Façon de travailler ensemble` và
`Transmission & intégration`.

Ba value là descriptive Restaurant Knowledge, độc lập và optional; all-empty là
hợp lệ. Initial behavior chỉ gồm manual input, view, edit, một explicit save cho
toàn bộ slice và no autosave. View consume Restaurant Knowledge READ; edit/save
consume MANAGE. OWNER và MANAGER có cả hai operation; STAFF bị deny mặc định.

Restaurant Knowledge giữ canonical ownership và establishment semantic scope;
Organization chỉ là tenancy/access envelope. Slice không sở hữu, đọc, ghi, liên
kết hoặc đồng bộ employee-specific data; không tạo Personnel, HR, Planning,
Pointage, Today, Tâches du jour, Formalités, onboarding/training, POS/Site
Agent, Display hoặc provider dependency. Proposal không quyết định schema,
table, repository, API, shared contract hoặc Product validation.
...
Shared worktree đã dirty với implementation, normative specs, archives và
review evidence của các Restaurant Knowledge slice trước khi change này được
tạo. Gate 1 giữ nguyên các path đó. Attribution của run này giới hạn ở new
OpenSpec directory và review packet directory; không runtime, schema, migration,
Product Knowledge, normative spec, Design, Tasks hoặc implementation file nào
được thay đổi.

## Authorities consulted

- Root, Backoffice và cloud-database scoped instructions.
- Authority Model, OpenSpec activation/normativity policies, Lifecycle Status
  Model và Restaurant Knowledge Module Registry row.
- ADR-007, Establishment Product Knowledge, `Informations générales` Product
  Knowledge, Personnel và Today Product Knowledge.
- Tenancy, Authentication, Identity/Membership và Database Boundaries.
- Normative specs cho Restaurant Knowledge authorization, Concept/Histoire,
  Cuisine/savoir-faire và Expérience client.
- Current general-information page pack.
- Current page composition, authorization guards, loaders/actions/forms,
  Restaurant Knowledge schema/repository và focused authorization/tenant tests.

## Current compatibility finding

Existing Restaurant Knowledge infrastructure tương thích với approved slice mà
không đổi accepted boundary:

- READ và MANAGE đã tồn tại dưới dạng operation riêng, với grant
  OWNER/MANAGER và STAFF denial đã accepted.
- Trusted active organization + establishment membership enforcement vẫn
  server-derived và fail closed.
- Current Restaurant Knowledge reads/writes được scope bằng cả organization và
  establishment.
- Page đã compose ba independent Restaurant Knowledge slices mà không hợp nhất
  chúng vào Establishment Profile.
- Team/culture chưa có schema field, repository operation, loader, action,
  form/model hoặc test.
- Scoped inspection không phát hiện dependency bắt buộc với Personnel,
  Planning, Pointage, Today, Tâches du jour, Formalités, onboarding/training,
  POS/Site Agent, Display hoặc external provider.

Các implementation hiện có là compatibility evidence, không phải authority để
copy technical representation một cách cơ học.

## Mandatory Cross-Module second-line result

Cả 10 v3 impact dimensions đã được reassess trong Analysis. Không có read/write
sang module-owned data khác; canonical ownership không đổi; không có consumer
hoặc reactive dependency; shared READ/MANAGE và trusted tenancy chỉ được
consume không đổi; chỉ một cloud runtime và một page bị ảnh hưởng; không có
legal/privacy/provider dependency mới; không thay ADR/durable boundary; không
cần coordinated Product decision, rollout contract hoặc multi-page QA.

Current classification: `PAGE_LOCAL`.

Nếu Design/Apply cần bất kỳ shared permission/contract, employee data/state,
Personnel/Planning/Pointage/Today/Tâches du jour/Formalités dependency,
cross-runtime behavior, privacy/legal decision hoặc provider nào, workflow phải
dừng với `CROSS_MODULE` / `NEEDS REVIEW`.

## Conflicts and NEEDS REVIEW

- **CONFLICT — non-blocking documentation drift:** broad
  `docs/PRODUCT_KNOWLEDGE.md` vẫn gọi Restaurant Knowledge là not-started với
  ownership/permissions chưa giải quyết. Module Registry, page Product
  Knowledge, current normative specs và workspace implementation cho thấy
  accepted state hiện tại đã tiến xa hơn. Drift này không thay đổi requirements
  của `Équipe & culture` và chỉ được sửa trong later approved documentation
  scope.
- **NEEDS REVIEW — deferred to Design:** exact persistence shape,
  table/repository representation, page-local component boundary và
  migration/rollback mechanics không phải Product decisions. Design chỉ được
  chọn chúng bên trong accepted Restaurant Knowledge cloud/domain boundary.
- Không có remaining requirement-level ambiguity, authorization gap,
  cross-module dependency hoặc durable-boundary conflict block Specs.

## Product/authority questions requiring answers

Không có câu hỏi cần trả lời trước Specs. Nếu later Design hoặc Apply cần shared
contract, permission mới, tenant semantics hoặc canonical ownership thay đổi,
employee-specific persistence/state, prohibited module dependency,
legal/privacy decision, external provider hoặc cross-runtime behavior, change
phải dừng và quay lại review.

## Lifecycle baseline

Restaurant Knowledge giữ nguyên:
...
Gate 1 không promote bất kỳ value nào.

## Analysis conclusion and recommendation

Conclusion: `READY_FOR_SPECS`.

Recommendation: approve Gate 1 để cho phép tạo duy nhất delta spec cho
`restaurant-knowledge/team-culture`. Không tạo Design, Tasks hoặc
implementation trước các normal approval gates tương ứng.
