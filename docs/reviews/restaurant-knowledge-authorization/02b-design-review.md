...
Gate: 2b — Mandatory Sensitive Design Review
Review status: APPROVED
Created: 2026-08-30T21:42:00.9248750+02:00
Schema: yuta-spec-driven
Analysis conclusion: READY_FOR_SPECS
Sensitive change: YES — authorization/security và CROSS_MODULE durable boundary
Approval source: explicit current-user instruction
Approval recorded by: Codex workflow
Approved: 2026-08-30T21:52:12.3699282+02:00
...
- Analysis hash: `88a4bf5b6ba44ca4be4fe631d04d49b867b6685f8c689a3c6e0ccb6c9c3cf6ff`
- Proposal hash: `356b47691970101a0e760da13d559895d24d7347cb177cdbd97c86c32259d3fe`

### Gate 2

- Packet: [`02-specs-review.md`](02-specs-review.md)
- Status: `APPROVED`
- Approved: `2026-08-30T21:41:05.8311656+02:00`
- Delta spec hash: `41ccb74dffcd56f5c23760a4bb11efbbf4363697feb1651980c27ef5134c67e0`

All earlier artifact path-sets and hashes were recomputed before Design and matched exactly.
...
| `openspec/changes/restaurant-knowledge-authorization/design.md`                                        | `0acf4049546b8eb8df144fdd4b8bad8d499b20f6cd9381faf329820b134cfcf2` |
...
- Security-sensitive: adds two new module operation mappings and fail-closed guards; no grants beyond `OWNER`/`MANAGER`.
- Existing active session, active organization, active establishment and matching active membership validation remains upstream and unchanged.
- `YUTA_ADMIN`/`YUTA_SUPPORT` stay outside restaurant membership grants; no bypass path is added.
- No database schema, migration, data write, persistence owner, API, route, UI or runtime topology change.
- Establishment Profile and all unrelated permission contracts remain unchanged.
- Future callers must use trusted `TenantContext` and exact READ/MANAGE guard at the server boundary; client visibility is never authorization.

## Migration and rollback review

- Migration: code-only typed permissions, grant map, guards, focused tests and bounded authority-document alignment.
- Database/data migration: none.
- Activation: prerequisite alone exposes no Restaurant Knowledge API/UI and performs no content operation.
- Rollback: remove the exact permission type/map/guards/tests and bounded docs. No data rollback is required.
- Coordinated rollback: if a dependent change consumes these guards, rollback must include or precede dependent integration removal; fail closed is mandatory.

## Unresolved choices

None that affect specs, approach or task breakdown. Exact implementation is fully bounded by the reviewed design:

- existing `permissions.ts` integration point;
- exact `restaurant-knowledge.read` and `restaurant-knowledge.manage` literals;
- separate grant entries;
- `has`/`require` guards;
- focused Backoffice authorization test;
- no route/session wrapper or new framework.
...
Approve this mandatory Design Gate for the exact design and hashes above. Only after explicit Design approval may the workflow create `tasks.md` and proceed toward Apply.

No Tasks or implementation have been created.
