# Gate 2 Review — reputation-review-social-links-configuration
...
Approved: `2026-09-05T19:44:20.9688670+02:00`

Created: `2026-09-05T17:57:29.9805333+02:00`

Regenerated: `2026-09-05T19:32:53.5790801+02:00`
...
Technical Design: `AUTHORIZED`

Sensitive Design: `NOT_STARTED`
...
The previous Gate 2 packet with SHA-256
`ed72839b2bb7bd0c441c0bbe2b34b1710ddac59cf851e9b3df228254b2a11168`
and exact Spec SHA-256
`392690594a771d81352f3770562eb75be57317d73ee191bb9aa3a6fdf49a870d`
(15 Requirements / 99 Scenarios) was `APPROVED` by explicit current-user
instruction. Technical Design then stopped at
`MISSING_SETTINGS_ROW_BEHAVIOR_NEEDS_REVIEW`.

The current user approved a targeted behavioral amendment only:
`MISSING_SETTINGS_ROW_BEHAVIOR` = `FAIL_CLOSED / NO_AUTO_CREATE`, with
provisioning `OUT_OF_SCOPE / SEPARATE_PRODUCT_DECISION`. This packet supersedes
the previous Gate 2 packet only for that amendment and awaits reapproval.

The prior Gate 2 packet with SHA-256
`e37af97c67f4fff748316c562806a11703c3cd07bc6effa9abacc20cfb28c164`
received
`CHANGES_REQUESTED — TARGETED PROVIDER HOST CLARIFICATION ONLY`.

This regenerated packet supersedes only the provider-host wording and scenarios
covered by G2-R1 and G2-R2. All other 15 Requirement semantics remain
unchanged.

## Targeted correction table

| Item                                               | Requested clarification                                                                                                       | Regenerated result                                                                                                                                                                                                                                                                                                                          |
| -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G2-R1 — explicit Google host allowlist             | Replace the unresolved Maps-capable subdomain class with an exact first-slice set and accept standard `www.google.com/maps/`. | Exactly seven hosts are recognized: `g.page`, `maps.app.goo.gl`, `google.com`, `www.google.com`, `google.fr`, `www.google.fr`, and `search.google.com`. The first two retain normal path/query; the four Google/France hosts require `/maps/`; `search.google.com` requires `/local/writereview`. Every other Google hostname fails closed. |
| G2-R2 — explicit Facebook/Instagram host allowlist | Remove ambiguous “approved dot-bound subdomain” wording.                                                                      | Facebook accepts exactly `facebook.com`, `www.facebook.com`, `m.facebook.com`, and `fb.me`. Instagram accepts exactly `instagram.com` and `www.instagram.com`. Every other subdomain fails closed.                                                                                                                                          |
| G2-R3 — missing settings row behavior              | Define behavior when no scoped `reputation_settings` row exists without expanding provisioning ownership.                     | Private read and Save fail closed with configuration unavailable; no data is synthesized, no row is created/upserted, no links or audit are written, no unrelated Reputation field is defaulted, and later external provisioning can be observed only after reload.                                                                         |

No wildcard provider hostname behavior remains.

## Approved Gate 1 authority

| Artifact                                                                          | Approved/current SHA-256                                           | Result          |
| --------------------------------------------------------------------------------- | ------------------------------------------------------------------ | --------------- |
| `openspec/changes/reputation-review-social-links-configuration/proposal.md`       | `12f138ad7de17186a313e14a08cb26f4f06333be2a03f8fc2445a63f8338ad61` | MATCH           |
| `openspec/changes/reputation-review-social-links-configuration/analysis.md`       | `02c0213d17c754b3617738da4c4ef04aca4566e3ec42d669192cbe086b2a1f4d` | MATCH           |
| `docs/reviews/reputation-review-social-links-configuration/01-analysis-review.md` | `2106185211b2db18a5164a2c721b14a08bba89be22a8f80f03abdbd9c74fe873` | APPROVED packet |

Proposal, Analysis and Gate 1 bytes are unchanged by G2-R1/G2-R2 and the
targeted missing-row amendment.

Gate 1 classification remains `CROSS_MODULE`. Authorization prerequisite,
audit prerequisite and schema/migration remain `NOT_REQUIRED`.

## Delta Spec inventory

| Capability                                     | Operation | Path                                                                                                                       | SHA-256                                                            | Requirements | Scenarios |
| ---------------------------------------------- | --------- | -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | -----------: | --------: |
| `reputation/review-social-links-configuration` | ADDED     | `openspec/changes/reputation-review-social-links-configuration/specs/reputation/review-social-links-configuration/spec.md` | `ba36028f4d8461ca8f8742eff00d81ad14e45ee14ffbabc0b4787749679dad07` |           15 |       103 |

No unrelated main Spec or second delta capability was created or modified.

## Gate 1 decision → Requirement mapping

| Approved decision/boundary                                                                                                              | Spec requirement                                                               |
| --------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| Reputation ownership; trusted organization + active establishment; browser claims cannot create authority; unrelated runtime boundaries | `Reputation sở hữu cấu hình theo trusted organization và active establishment` |
| Missing scoped settings row fails closed; no implicit creation/defaults; external provisioning remains separate                         | `Reputation sở hữu cấu hình theo trusted organization và active establishment` |
| Base `reputation.read`; OWNER-only `reputation.settings.manage`; no MANAGER/STAFF or substitute permission                              | `Settings dùng authority Reputation hiện hành và chỉ OWNER được truy cập`      |
| Exactly three nullable values; no generic entity/status                                                                                 | `Capability chỉ quản lý đúng ba giá trị nullable`                              |
| Add/replace/remove, mixed mutation, explicit Save, no autosave                                                                          | `Thay đổi chỉ xảy ra qua một explicit Save`                                    |
| Multi-field atomicity and failure preservation                                                                                          | `Save nhiều field là all-or-nothing`                                           |
| Normalized no-op and authoritative success reload                                                                                       | `Normalized no-op không ghi và success trả authoritative state`                |
| No silent last-write-wins; recoverable stale conflict/reload; response-loss replay                                                      | `Stale conflict và response-loss replay phải recoverable`                      |
| Common HTTPS/trim/blank/length/userinfo/no-network/no-redirect rules                                                                    | `Mọi provider URL tuân thủ normalization và safety chung`                      |
| Corrected explicit Google review/Maps host-purpose policy                                                                               | `Google URL chỉ chấp nhận destination phục vụ review hoặc Maps`                |
| Explicit Facebook host allowlist                                                                                                        | `Facebook URL có bounded provider-host policy`                                 |
| Explicit Instagram host allowlist                                                                                                       | `Instagram URL có bounded provider-host policy`                                |
| Manual-only Google URL and GBP independence                                                                                             | `Google review URL là manual-only và độc lập GBP connector`                    |
| Trusted public consumer, fail-closed legacy values and safe anchor attributes                                                           | `feedback-web chỉ render safe configured CTA theo trusted public scope`        |
| Actual-change-only atomic SETTINGS audit and forbidden audit content                                                                    | `Real mutation và SETTINGS audit phải thành công hoặc thất bại cùng nhau`      |
| Provider/workflow/production exclusions                                                                                                 | `Capability không mở rộng sang provider workflow hoặc production operation`    |

## G2-R1 Google mapping

Normative exact host set:

- `g.page`: HTTPS, normal approved path/query behavior;
- `maps.app.goo.gl`: HTTPS, normal approved path/query behavior;
- `google.com`, `www.google.com`, `google.fr`,
  `www.google.fr`: HTTPS and pathname starts `/maps/`;
- `search.google.com`: HTTPS and pathname starts
  `/local/writereview`;
- all other Google hostnames/subdomains: fail closed.

Explicit accept scenarios cover all seven hosts/classes, including
`www.google.com` and `www.google.fr`. Explicit rejection covers
`mail.google.com`, `accounts.google.com`, `docs.google.com`, unlisted
Google subdomains including artificial `/maps/`, `google.com` without
`/maps/`, lookalikes, generic shorteners, HTTP and embedded credentials.
`maps.google.com` is not added.

## G2-R2 Facebook/Instagram mapping

Facebook accepts exactly:
...
Instagram accepts exactly:

- `instagram.com`;
- `www.instagram.com`.

Explicit scenarios cover every accepted host and rejection of unlisted
provider subdomains. Existing HTTPS-only, userinfo, lookalike and no
network/redirect requirements remain unchanged. No OAuth or path-specific review
verification is added.

## Authorization mapping

- OWNER allow: Requirement 2, scenario `OWNER được xem và quản lý settings`.
- MANAGER deny: Requirement 2, scenario `MANAGER không thấy settings surface`.
- STAFF deny: Requirement 2, scenario `STAFF không thấy settings surface`.
- Browser claims denied: Requirements 1–2.
- Wrong establishment fail closed: Requirement 1.
- Connector/review/marketing/Establishment permissions cannot substitute:
  Requirement 2.
- The Spec preserves current permission meanings and does not redefine the grant
  map.

## Public consumer mapping

Requirement `feedback-web chỉ render safe configured CTA theo trusted public
scope` covers independently visible Google/Facebook/Instagram CTAs, null
hiding, malformed/unsupported legacy hiding, trusted server-resolved public
scope, rejection of browser tenant authority and exact
`target="_blank"`/`rel="noopener noreferrer"` behavior.

The explicit host allowlists now provide deterministic fail-closed treatment of
legacy values on every unlisted provider subdomain.

## Audit mapping

Requirement `Real mutation và SETTINGS audit phải thành công hoặc thất bại
cùng nhau` covers one mutation-level SETTINGS audit, multi-provider deltas,
actor/trusted scope/previous-new/timestamp, settings-audit coherence, and no
audit for no-op, invalid input, stale rejection or failed operation. It
explicitly excludes tokens, credentials, rejected URL userinfo, request dumps,
IP and user-agent.

## Concurrency and retry mapping

Requirement `Stale conflict và response-loss replay phải recoverable` defines
only observable behavior:

- no silent last-write-wins;
- recognizable conflict for materially different stale save;
- authoritative state remains unchanged on rejection;
- reload/recovery;
- equivalent replay after lost response creates no duplicate effect/audit;
- materially different retry is not equivalent.

It does not prescribe timestamp/value CAS, command receipts, request IDs,
operation keys or a database primitive.

## Explicit exclusions coverage

Requirement `Capability không mở rộng sang provider workflow hoặc production
operation` expressly excludes Google OAuth expansion, Facebook/Instagram OAuth,
review import/aggregation, social publishing, AI, analytics, QR, generic
social-link engine, automatic derivation, provider network verification,
redirect resolution, unrelated refactors, migration, production data mutation,
route production enablement and deployment. Requirements 1 and 12 additionally
preserve runtime and GBP boundaries.

## Requirement/scenario completeness

All minimum requested states remain explicit:

- empty and populated;
- add, replace, remove, mixed multi-field save and no autosave;
- all-or-nothing, no-op, successful reload and server failure;
- invalid Google/Facebook/Instagram, userinfo, HTTP, lookalike, shortener and
  over-length input;
- stale conflict/reload, response-loss replay and materially different retry;
- correct/wrong tenant-establishment and OWNER/MANAGER/STAFF behavior;
- missing scoped settings row on private read/Save, disabled Save, no synthesized
  data, no implicit row/field creation, and recovery after separate provisioning;
- legacy unsafe public URL hiding and safe anchor attributes;
- audit success plus no audit for no-op, invalid input, stale reject or failed
  operation.

G2-R1/G2-R2 add exact positive/negative provider-host scenarios. The targeted
missing-row amendment adds four fail-closed scenarios without changing the
other approved behavioral families.

## Remaining ambiguity

Provider hostname ambiguity from the prior packet is resolved by the exact
first-slice allowlists. No additional Google, Facebook or Instagram subdomain is
implicitly eligible.

No Product, authorization, audit, ownership, tenancy, schema or runtime
ambiguity remains inside this bounded capability. Universal
`reputation_settings` provisioning remains an explicit out-of-scope readiness
dependency requiring a separate Product decision before a rollout that expects
...
`pnpm exec openspec validate reputation-review-social-links-configuration --strict`

Exit result: `0`

Exact result:

`Change 'reputation-review-social-links-configuration' is valid`

Scoped count command used PowerShell regular expressions over the exact Spec
bytes:
...
Path:
`openspec/changes/reputation-review-social-links-configuration/specs/reputation/review-social-links-configuration/spec.md`
...
SHA-256 was computed with PowerShell
`Get-FileHash -Algorithm SHA256 <path>` over exact current file bytes and
recorded in lowercase hexadecimal.

## Scope confirmation

- Proposal modified: `NO`
- Analysis modified: `NO`
- Gate 1 packet modified: `NO`
- Delta Spec files modified: `1`
- Gate 2 review packet regenerated: `YES`
- Main Specs modified: `NO`
- Design created: `NO`
- Tasks created: `NO`
- Contracts/application/UI/authorization modified: `NO`
- Schema/migration created or modified: `NO`
- Apply performed: `NO`
- Production operation: `NO`

## Amendment approval disposition

Previous Gate 2: `APPROVED`

Targeted amendment: `MISSING_SETTINGS_ROW_BEHAVIOR`

Decision: `FAIL_CLOSED / NO_AUTO_CREATE`

Provisioning: `OUT_OF_SCOPE / SEPARATE_PRODUCT_DECISION`

Gate 2 amendment: `APPROVED`
...
The current user approved the exact amended Spec SHA-256
`ba36028f4d8461ca8f8742eff00d81ad14e45ee14ffbabc0b4787749679dad07`
with 15 Requirements and 103 Scenarios and authorized Technical Design plus
the required Sensitive Design review packet only. Tasks, Apply and production
remain unauthorized.
