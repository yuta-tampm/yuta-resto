...
| Section/flow | `/management`, shared `ManagementHeader.tsx`, approved printing as-built evidence | APPROVED         | dark POS Management top header direction, YUTA POS identity, `Gestion locale`, user/role context, return-to-POS and account/sign-out behavior | compress labels on small widths using the existing responsive pattern       | left sidebar, mobile drawer, persistent module tabs, invented modules        | Product owner approved this shared direction on 2026-08-11. Runtime reuse was completed in Phase 1 on 2026-08-12. |
...
`apps/yuta-pos/src/app/management/_components/ManagementHeader.tsx` and
