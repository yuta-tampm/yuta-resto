...
import { hasEmployeeCreateUnsavedInput } from '../_lib/employee-create-flow';
import { formatEmployeeDate, getEmployeeDossierHref } from '../salaries-model';
...
export function EmployeeCreateDialog({
  ...
  const hasUnsavedInput = hasEmployeeCreateUnsavedInput({
    givenNames,
    familyName,
    position,
    qualification,
  ...
  const employeeDossierHref = state.employeeId
    ? getEmployeeDossierHref(state.employeeId)
    : null;
  ...
  function handleOpenChange(nextOpen: boolean) {
    ...
    if (state.status !== 'success' && hasUnsavedInput) {
  ...
  return (
    <>
        ...
        <DialogContent className="max-h-[92vh] max-w-2xl overflow-y-auto">
          ...
          <form action={formAction} className="mt-5 grid gap-5">
            ...
            <section
              ...
              aria-labelledby="employee-employment"
            >
                  ...
                  {state.duplicateCandidates.map((candidate) => (
                    ...
                    >
                      ...
                      <span className="block text-secondary">
                        Entrée :{' '}
                        {formatEmployeeDate(candidate.entryDate, locale)}
                      </span>
                    </li>
                  ))}
                </ul>
            ...
            <DialogFooter>
              ...
              {state.status === 'success' && employeeDossierHref && (
                <Button asChild>
                  <Link href={employeeDossierHref}>Ouvrir le dossier</Link>
                </Button>
              )}
              {state.status !== 'success' && (
      ...
      <ConfirmDialog
        ...
        description="Les informations saisies ne sont pas enregistrées et seront perdues."
